...
def get_excluded_folder_names() -> Set[str]:
    """
    Returns a set of lowercase folder names that should be excluded from scanning
    (e.g., 'selection', 'selected', 'trash', plus the custom selection folder from config).
    """
    excluded_names = {"selection", "selected", "trash"}
    try:
