...
class ScoreCache:
    ...
    def get_scores(self, filepath: Path) -> Dict[str, Union[float, str]]:
        ...
        """
        try:
            filepath_str = os.path.abspath(filepath)  # Bolt: Faster than Path.resolve()
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
    ...
    def get_multiple_scores(
        ...
        try:
            ...
            path_map = {os.path.abspath(p): p for p in filepaths}  # Bolt: Faster than Path.resolve()
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                if results:
                    matched_fps = [os.path.abspath(p) for p in results.keys()]  # Bolt: Faster than Path.resolve()
    ...
    def set_multiple_scores(
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                path_map = {os.path.abspath(p): p for p in scores_dict.keys()}  # Bolt: Faster than Path.resolve()
