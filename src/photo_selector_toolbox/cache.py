...
class ScoreCache:
    ...
    def get_scores(self, filepath: Path) -> Dict[str, Union[float, str]]:
        ...
        """
        try:
            # PERFORMANCE: os.path.abspath() is significantly faster than Path.resolve()
            # because it avoids disk I/O to check for symlinks
    ...
    def set_scores(self, filepath: Path, scores: Dict[str, Union[float, str]]) -> None:
        ...
        try:
            # PERFORMANCE: Use string lexical conversion over Path.resolve() disk I/O
            filepath_str = os.path.abspath(filepath)
            now = int(time.time())
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
