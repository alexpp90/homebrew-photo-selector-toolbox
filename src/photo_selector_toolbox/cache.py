...
class ScoreCache:
    ...
    def set_scores(self, filepath: Path, scores: Dict[str, Union[float, str]]) -> None:
        ...
        try:
            ...
            now = int(time.time())
            with sqlite3.connect(self.db_path) as conn:
                # Performance optimization: Use SQLite native json_patch function
                # to perform an in-database merge of JSON scores. This avoids an
                # N+1 bottleneck of fetching, deserializing, modifying, and re-serializing
                # the existing scores in Python.
                conn.execute(
                    """
