...
## 2024-05-18 - Discoverability of Keyboard Shortcuts in Destructive Actions\n**Learning:** When users face a destructive action dialog (like "Confirm Delete"), they may hesitate if they cannot see a quick keyboard exit or confirm path. By adding explicit shortcut hints (e.g., "(Delete)" or "(Esc)") to the button labels, the UI reduces friction and teaches power users efficient workflows.\n**Action:** Always consider appending standard shortcut hints in parentheses to the main action buttons in modal dialogues, especially for potentially destructive or frequent actions.
