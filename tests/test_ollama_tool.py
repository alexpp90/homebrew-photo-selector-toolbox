...
def test_ollama_tool_success(mock_opener_open, dummy_image_file, temp_config_dir):
    ...
    mock_opener_open.return_value = mock_response
    ...
    mock_opener_open.assert_called_once()
    # Check that URL used default
    req = mock_opener_open.call_args[0][0]
    assert req.full_url == "http://localhost:11434/api/generate"
    assert req.method == "POST"


@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_custom_config(mock_opener_open, dummy_image_file, temp_config_dir):
...
@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_connection_error(mock_opener_open, dummy_image_file, temp_config_dir):
    # Mock connection failure (URLError)
    mock_opener_open.side_effect = urllib.error.URLError("Connection refused")
...
@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_parsing_error(mock_opener_open, dummy_image_file, temp_config_dir):
...
@patch("urllib.request.OpenerDirector.open")
def test_ollama_tool_fallback_analysis(mock_opener_open, dummy_image_file, temp_config_dir):
