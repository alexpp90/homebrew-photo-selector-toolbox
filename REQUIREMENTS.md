...
*   **Google Drive Scope Policy (`drive.file` ONLY):** Photo-Tok must request only the non-restricted `drive.file` OAuth scope (plus basic profile/email from sign-in). The full `drive` and `drive.readonly` scopes are *restricted* scopes (Google verification + annual CASA security assessment) and must never be requested. Consequences that the architecture must respect:
    ...
    - Picker configuration (API key `PHOTOTOK_PICKER_API_KEY`, Cloud project number `PHOTOTOK_GCP_PROJECT_NUMBER`) is injected via `BuildConfig`; when missing, the picker shows a "not configured in this build" error instead of crashing. These variables can be configured on GitHub as either secrets or variables, and can be configured locally for developer builds in `android/local.properties` as `phototok.picker.api.key` and `phototok.gcp.project.number`. Values are injected into the picker HTML via `JSONObject.quote` (never raw string interpolation).
