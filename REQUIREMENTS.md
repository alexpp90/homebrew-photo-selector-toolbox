...
*   **CI Workflow:** `.github/workflows/android.yml` — runs on `ubuntu-latest` with JDK 17. Triggers on pushes to `main`, tags matching `v*`, and PRs modifying `android/**`. Runs lint and unit tests before building. Produces signed APKs and AABs for both the `:app` (Photo Selector Toolbox) and `:phototok` (Photo Tok) modules.
