...
*   **Window Size Classes:** Every screen must support three `WindowWidthSizeClass` breakpoints:
    ...
    *   **Focused Layout Space Efficiency:** The focused layout must maximise the current image on tablet-class screens (reference device: Galaxy Tab S11 Ultra, 16:10, predominantly 4:3 photos). A 4:3 image fitted into a full-width top region is height-limited and leaves an empty letterbox column on each side; the controls must occupy that dead space as two 56dp-wide vertical rails — navigation and position counter on the left, image actions (Move, Copy, Delete, Fullscreen, layout toggle, details toggle, filmstrip toggle) on the right — so they consume no vertical space and never cover the photo. The current image takes ~72% of the free height (~80% on a Compact height window) with the Previous/Next tiles below; outer padding is kept to ≤6dp. The candidate filmstrip and the photo-details overlay are both collapsible from the action rail so the image can use the full screen.
    *   **First-Run Navigation Hint:** On-image Previous/Next arrow overlays are a one-time affordance only: they are shown on the first run (persisted `hasSeenNavHint` flag) to indicate tappable navigation, then suppressed on subsequent runs to keep images unobstructed. Navigation thereafter relies on tapping the Previous/Next tiles, hardware keys, and drag gestures.
...
*   **Score Badge Legibility (Toolbox / Android Desktop):** Post-scan quality badges must not be bare icon-and-number pairs. Every metric is defined once in `com.photoselectortoolbox.domain.scoring.ScoreMetric` (short chip label, display name, plain-language description, value format, and whether higher or lower is better) and rendered via the shared `ScoreChip`/`ScoreChipRow` components. Chips show icon + short label + formatted value; the label may be dropped only where space is genuinely unavailable (thumbnail overlays), in which case the accessibility description still carries the full name, value and direction. Value formatting must be `Locale.US` so decimal separators do not vary by device locale. An info action in the selector app bar — shown once any image has scores — opens a legend sheet explaining every metric and its direction.
...
*   **Controls Guide (PhotoTok / Android Phone):** An info button in the viewer (portrait top app bar and landscape left side panel) opens a full-screen guide listing every available control — vertical swipe, swipe right, swipe left, single tap, double tap/pinch, logo tap, Selection, Revert, Sources, Settings — with what each one does. The guide is the same composable as the first-launch gesture tutorial and is parameterised by the configured swipe actions and folder names, so it must never describe a left swipe as "delete" when the user has configured copy or move. A "Reset Tutorials" entry in Settings clears the seen-hint keys and the tutorial timestamp so all guidance reappears.
