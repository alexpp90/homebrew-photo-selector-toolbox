...
|----------|-------------------|-----------|
| `SUPPORTED_EXTENSIONS` | `exif/reader.py` | `core/duplicates.py`, `gui/app.py`, `cli.py` |
| `FORCE_EXIFTOOL_EXTENSIONS` | `exif/reader.py` | Internal to `exif/reader.py` |
| RAW extensions for preview/sharpness | Should import from `exif/reader.py` | `core/utils.py`, `core/sharpness.py` |
...
**Rule:** The output of `get_exif_data()` must return a typed `ExifData` dataclass instance (defined in `core/models.py`) rather than a raw dictionary:
