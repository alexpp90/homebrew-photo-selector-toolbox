...
| `assembleDebugAndroidTest` | `android.yml:unit-tests` | ✅ needs JDK 17 + Android SDK | Compiles `src/androidTest`. Nothing else does. |
...
`tests/conftest.py` skips tests by platform (`linux_only`, `mac_only`,
...
Baselines in `tests/visual/baselines/` are rendered under Xvfb. macOS and
...
`android/app/src/androidTest/` and `android/phototok/src/androidTest/` assert
