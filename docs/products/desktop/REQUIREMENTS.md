...
> Scope: this file is authoritative for the desktop application only. Requirements shared
> with the Android products live in [`../../shared/`](../../shared/); Android requirements
> live under [`../android-desktop/`](../android-desktop/) and [`../phototok/`](../phototok/).
...
*   **Supported File Types:** Supported file extensions are defined centrally in `products/desktop/src/photo_selector_toolbox/exif/reader.py` as `SUPPORTED_EXTENSIONS`. Other tools (like `duplicates.py`) must import this list and may extend it locally (e.g., appending `.bmp` and `.gif`).
...
*   **ExifTool Bundling:** The build script (`products/desktop/scripts/build.py`) relies on a hardcoded ExifTool version (e.g., 13.51) downloaded from SourceForge. This requires manual updates if SourceForge removes older releases. Windows builds must utilize the `_64` suffixed ExifTool binary.
...
*   **Auto-Update:** The CI workflow automatically updates both Cask and Formula configurations using a Python utility script `products/desktop/scripts/update_formula.py`:
...
*   **GUI Unit Tests:** Tests validating GUI components (e.g., `products/desktop/tests/unit/gui/test_sharpness_gui_basic.py`) require extensive mocking of `tkinter`, `PIL`, and `photo_selector_toolbox` dependencies due to the lack of a display environment in CI runners.
