...
3. Get each app's **Play App Signing SHA-1** (only if the app already exists in Play):
   ...
   - Do this for **Photo Selector Toolbox** now; do it for **PhotoTok** after you
...
## Phase 2 — Create the PhotoTok project  🔥

1. 🔥 Firebase Console → **Add project**.
2. Name it `PhotoTok`. When it shows the project ID, click the edit pencil and set
...
## Phase 3 — PhotoTok: enable Drive API + OAuth  ☁️

> The Google Cloud Console and Firebase share the **same** project; just switch the
> project picker at the top to **PhotoTok / phototok-app**.
...
2. **OAuth consent screen**
   ...
   - App name `PhotoTok`, support email = your email, developer contact = your email.
...
3. **Android OAuth client**
   ...
   - Come back after Phase 5 and **add the PhotoTok Play App Signing SHA-1** to
     this same client.
...
## Phase 4 — PhotoTok: service accounts  ☁️
...
## Phase 5 — PhotoTok: create the Play app + grant access  ▶️

1. ▶️ Play Console → **Create app**.
   - App name `PhotoTok`, default language, **App or Game = App**, free/paid as you
...
2. **Grant the publisher service account access**
   ...
   - **App permissions** → select **PhotoTok only**.
   - Permissions → at minimum **Release to testing tracks** (and **View app
     information**). Invite.
3. **Get the PhotoTok Play App Signing SHA-1** (now that the app exists)
...
✓ **Checkpoint:** PhotoTok app exists in Play, the `play-publisher` service account
is invited with testing-track access scoped to PhotoTok.

---

## Phase 6 — Clean up the Toolbox project  🔥 / ☁️

The Toolbox keeps the existing project `photo-selector-tb-dist`. Strip PhotoTok out
of it so it's Toolbox-only.

1. 🔥 Firebase Console → switch to **photo-selector-tb-dist** → Project settings →
   **Your apps** → find the old PhotoTok app
...
3. ▶️ Toolbox **upload key** — depends on whether the app has ever been released:
   ...
   - **Only if Toolbox is ALREADY live under a different (old shared) key** do you
     ...
     two to take effect).
   - **PhotoTok** is brand-new, so it always uses the "do nothing" path: its key is
...
2. Watch these steps:
   ...
   - **Upload … to Firebase App Distribution** → Toolbox to its project, PhotoTok
