---
name: android_ui_agent
description: "Android Jetpack Compose UI specialist for all files under android/app/src/main/.../ui/. Handles Material 3 theming, adaptive layouts (phone/tablet/DeX), navigation, gesture handling, and ViewModels."
---
...
8. **Compose instrumented-test conventions.** Instrumented tests need an emulator, so a wrong assertion costs a full CI round trip — the score-legend branch burned nine commits, eight of them this one family of mistakes. Before writing a Compose UI test, read `docs/CI_PARITY.md` § *Compose instrumented-test conventions*, and apply:
   - Use `onAllNodes(...).onFirst()` rather than `onNode(...)` whenever a matcher can legitimately hit more than one node. `onNode` throws on multiple matches, and Compose duplicates nodes more often than it appears.
   - Pass `useUnmergedTree = true` when asserting on content inside a semantics-merging container (`ModalBottomSheet`, list rows, icon+label buttons). Without it the child node is not in the tree you are querying.
   - Scope assertions with `hasAnyAncestor(hasTestTag("..."))` instead of matching bare text globally — bare `hasText("Sharpness")` matches both the chip and the legend row.
   - **Import Compose test matchers explicitly; never `import androidx.compose.ui.test.*`.** The wildcard is what let the non-existent `hasTag` (the real matcher is `hasTestTag`) pass review and fail only inside the emulator job.
9. **Always compile instrumented tests before pushing.** Run `./scripts/run_tests.sh --android`. It runs `assembleDebugAndroidTest`, the only task that compiles `src/androidTest` — neither `testDebugUnitTest` nor `lintDebug` does. Skipping it means a Kotlin typo in a test surfaces in CI as a bare `The process '/usr/bin/sh' failed with exit code 1`.
